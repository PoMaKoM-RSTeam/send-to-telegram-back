...
const questionForChangeBotName = new StatelessQuestion('question2', async (ctx, additionalState) => {
  await editBotInDataBase(ctx, models.default.botModel, additionalState, ctx.message.text);
...
const questionForChangeChannelName = new StatelessQuestion('question3', async (ctx, additionalState) => {
  await editChannelInDataBase(ctx, models.default.channelModel, additionalState, ctx.message.text);
  await ctx.reply('Done!', { reply_markup: { remove_keyboard: true } });
});
...
const question = new StatelessQuestion('question', async (ctx, additionalState) => {
  ...
  if (additionalState === 'botAdd') {
    ...
    console.log(ctx.update.message.from.id);
    // newwwbot.drop();
    await saveBotToDataBase(ctx, models.default.botModel, ctx.message.text).save();
    await ctx.reply('Done!', { reply_markup: { remove_keyboard: true } });
    ...
    await deleteBotFromDataBase(ctx, models.default.botModel, ctx.message.text);
    await ctx.reply('Done!', { reply_markup: { remove_keyboard: true } });
  } else if (additionalState === 'channelAdd') {
    try {
      bot.api
        .getChat(`@${ctx.message.text}`)
        .then(async (channel) => saveChannelToDataBase(ctx, models.default.channelModel, channel).save());
    } catch (error) {
      console.log(error);
    }
    await ctx.reply('Done!', { reply_markup: { remove_keyboard: true } });
    ...
    await deleteChannelFromDataBase(ctx, models.default.channelModel, ctx.message.text);
    await ctx.reply('Done!', { reply_markup: { remove_keyboard: true } });
  }
});
...
bot.on('callback_query:data', async (ctx, next) => {
  ...
  if (ctx.callbackQuery.data.slice(ctx.callbackQuery.data.length - 18) === 'Bots/options2:add/') {
    ...
    const showAll = await showAllUserBotsFromDataBase(ctx, models.default.botModel);
    ...
    const showAll = await showAllUserChannelsFromDataBase(ctx, models.default.channelModel);
    await ctx.reply(showAll, { reply_markup: { remove_keyboard: true } });
